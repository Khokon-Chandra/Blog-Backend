<x-backend.app-layout>
    ...
    <div class="row">
        ...
        <div class="col-lg-8 col-md-8 col-12">
            ...
            <hr class="bg-theme mt-0 p-0 ">
            <?php
                if(isset($selectedMenu->menuItems)){
                    echo '<ul class=" pl-0" id="sortable">';
                    foreach ($selectedMenu->menuItems as $item) {
                       printf('<li data-id="%d" class=" bg-white border p-2 mb-1">%s<ul></ul></li>',$item->id,$item->name);
                    }
                   printf('</ul><div class="text-right"><button id="saveMenu" class="btn btn-primary">Save menu</button></div>');
                }else{
                    printf('<p>Add menu items form the column on the left</p>');
                }
            ?>
        </div>
    </div>
